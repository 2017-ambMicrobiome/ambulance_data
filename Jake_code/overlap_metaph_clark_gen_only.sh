...
echo "Genus,clark_relAbun,meta_relAbun" > ${out_d}/${out_file}_gen
echo -e "\nProcessing files ${in_clark} and ${in_meta}."
while read -r line || [[ -n "$line" ]]; do
	clark_gen_nm=$( echo "$line" | cut -d ',' -f1 );
	if [[ $clark_gen_nm != "Name" && $clark_gen_nm != "UNKNOWN" ]]; then
		overlap_chk_gen=$( egrep "g__${clark_gen_nm}" ${in_meta} | head -n 1);
		
		if [[ -n $overlap_chk_gen ]]; then
			echo -e "\n\t\tFound genus ${clark_gen_nm} in both files"
			clark_rel_abun=$( echo "$line" | cut -d ',' -f6 );
			meta_rel_abun=$( echo "$overlap_chk_gen" | cut -f2 );
			echo "${clark_gen_nm},${clark_rel_abun},${meta_rel_abun}" >> ${out_d}/${out_file}_gen
		fi
	fi		
done < ${in_clark}
