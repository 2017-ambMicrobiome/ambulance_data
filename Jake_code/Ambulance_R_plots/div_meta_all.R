...
amb_data <- read.table("/zenodotus/masonlab/rotcollab_scratch/har2011/ambulance/metaphlan_div/output/amb_meta.biom.tsv", sep = "\t", header = TRUE, numerals = c("allow.loss"), colClasses = "numeric")
...
sink("/home/jake/amb_stat.tex", append = TRUE, split = TRUE)
xtable(div_amb_stat)
...
write.csv(x3, file = "Diversity.indices.csv", quote = FALSE, row.names = FALSE)
