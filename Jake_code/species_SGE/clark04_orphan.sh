...
echo -e "\nclark04_orphan.sh started on ambulance samples at `date`.\nSyncing from: ${pathZeno} \nTo: ${TMPDIR}/clark01_work\n"
...
# 0-200
rsync -av $( find ${pathFromZeno}/*.gz | sort | tail -n 99 )  ${TMPDIR}/work
...
# Zipping clark04_orphan_out files
...
echo -e "\nFinished running clark04_orphan.sh at `date`."
