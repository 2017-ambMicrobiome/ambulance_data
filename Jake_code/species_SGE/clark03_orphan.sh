...
echo -e "\nclark03_orphan.sh started on ambulance samples at `date`.\nSyncing from: ${pathZeno} \nTo: ${TMPDIR}/clark01_work\n"
...
# 0-200
rsync -av $( find ${pathFromZeno}/*.gz | sort | head -n 300 | tail -n 100 )  ${TMPDIR}/work
...
# Zipping clark03_orphan_out files
...
echo -e "\nFinished running clark03_orphan.sh at `date`."
