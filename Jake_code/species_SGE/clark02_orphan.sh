...
echo -e "\nclark02_orphan.sh started on ambulance samples at `date`.\nSyncing from: ${pathZeno} \nTo: ${TMPDIR}/clark01_work\n"
...
# 0-200
rsync -av $( find ${pathFromZeno}/*.gz | sort | head -n 200 | tail -n 100 )  ${TMPDIR}/work
...
# Zipping clark02_orphan_out files
...
echo -e "\nFinished running clark02_orphan.sh at `date`."
