...
# Sync data to tmpdir
echo -e "\nrepair_PE6.sh started on ambulance samples at `date`.\nSyncing from: ${pathZeno} \nTo: ${TMPDIR}/working\n"
cd $TMPDIR
mkdir completed
mkdir working
mkdir soft
...
cd ${pathFromZeno}
rsync -av $( ls | egrep "AE|AS|AW" | sort | grep -o "A.*" | head -n 600 | tail -n 100 )  ${TMPDIR}/working
...
echo -e "\nFinished repair_PE6.sh at `date`."
