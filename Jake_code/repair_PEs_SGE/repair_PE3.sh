...
# Sync data to tmpdir
echo -e "\nrepair_PE3.sh started on ambulance samples at `date`.\nSyncing from: ${pathZeno} \nTo: ${TMPDIR}/working\n"
cd $TMPDIR
mkdir completed
mkdir working
mkdir soft
...
cd ${pathFromZeno}
rsync -av $( ls | egrep "AE|AS|AW" | sort | grep -o "A.*" | head -n 300 | tail -n 100 )  ${TMPDIR}/working
...
echo -e "\nFinished repair_PE3.sh at `date`."
